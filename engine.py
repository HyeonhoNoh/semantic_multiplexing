...
from base_args import PATCH_SIZE
...
def evaluate(ta_perform: str, net: torch.nn.Module, dataloader: Iterable, 
                  device: torch.device, criterion: torch.nn.Module, snr, img_name, print_freq=10):
    net.eval()
    if ta_perform.startswith('imgc'):
        ...
        with torch.no_grad():
            for batch_idx, (imgs, targets) in enumerate(dataloader):
                ...
                outputs = net(img=imgs, ta_perform=ta_perform, test_snr=snr)
            ...
            for batch_idx, (imgs, targets) in enumerate(dataloader):
                ...
                if batch_idx == 0:
                    ...
                    for i in range(4):
                        ...
                        axes[1,i].imshow(sample_output[i])
                        axes[1,i].set_title(f"Sample Outputs {i+1}")
                        axes[1,i].axis('off')
...
def train_class_batch_it(ta_perform, model, samples, targets, criterion, snr):
    if ta_perform.startswith('imgc'):
...
def train_class_batch_vqa(ta_perform, model, imgs, texts, targets, criterion, snr):
...
def train_epoch_vqa(model: torch.nn.Module, criterion: torch.nn.Module,
                ...
                device: torch.device, epoch: int, loss_scaler, ta_perform, max_norm: float=0,
                start_steps=None,lr_schedule_values=None, wd_schedule_values=None, 
                update_freq=None, print_freq=500, snr=torch.FloatTensor([12])):
    model.train(True)                                                         
    acc_meter = AverageMeter()
    loss_meter = AverageMeter()
    ...
    for data_iter_step, (imgs, texts, targets) in enumerate(data_loader):    
        ...
        loss, outputs = train_class_batch_vqa(
                ta_perform, model, imgs, texts, targets, criterion, snr=snr)
        loss_value = loss.item()
...
def train_epoch_msa(model: torch.nn.Module, criterion: torch.nn.Module,
                ...
                device: torch.device, epoch: int, loss_scaler, ta_perform, max_norm: float=0,
                start_steps=None,lr_schedule_values=None, wd_schedule_values=None, 
                update_freq=None, print_freq=5, snr=torch.FloatTensor([12])):
    model.train(True)                                                         
    acc_meter = AverageMeter()
    loss_meter = AverageMeter()
