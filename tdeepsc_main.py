...
def main(args):
    ...
    if args.is_single_label:
        ...
        img_folder = f"./results/imgs/SNR_{args.snr}_UE_{NUM_UE}_RATIO_{SHARE_RATIO}_sym_{args.n_sym_img}/"

    print(f"Images will be saved in {img_folder}")
    ...
    # If single label data is utilized
    # if args.is_single_label:
    #     indices = [i for i, label in enumerate(trainset.targets) if label in [0]]
    ...
    if args.ta_perform:
        ...
        # If single label data is utilized
        if args.is_single_label:
            indices = [i for i, label in enumerate(valset.targets) if label in [0]]
            valset = torch.utils.data.Subset(valset, indices)
    ...
    num_training_steps_per_epoch = (args.num_samples // total_batch_size) + 1
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.ta_perform.startswith('img') or args.ta_perform.startswith('text'):
            train_stats = train_epoch_it(
                    ...
                    update_freq=args.update_freq, snr=torch.FloatTensor([args.snr]))
        elif args.ta_perform.startswith('vqa'):
        ...
        if dataloader_val is not None:
            img_name = img_folder + f"sample_img_{epoch}.png"
            print(args.output_dir)
            if args.ta_perform.startswith('img') or args.ta_perform.startswith('text'):
                test_stats = evaluate(ta_perform=args.ta_perform, 
                                    net=model, dataloader=dataloader_val, 
                                    device=device, criterion=criterion, snr=torch.FloatTensor([args.snr]), img_name=img_name)
            elif args.ta_perform.startswith('vqa'):
                test_stats = evaluate_vqa(ta_perform=args.ta_perform, 
                                    net=model, dataloader=dataloader_val, 
                                    device=device, criterion=criterion)
