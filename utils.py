...
def get_model(args):
    ...
    model = create_model(
        ...
        n_sym_img=args.n_sym_img,
        n_sym_text=args.n_sym_text,
        n_sym_spe=args.n_sym_spe
    )
    # Model info logging
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    if args.ta_perform == "imgr":
        if args.is_single_label:
            output_dir = Path(args.output_dir + '/ckpt_' + args.ta_perform + '/SNR_' + str(args.snr) + '_UE_' + str(
                NUM_UE) + '_share_' + str(SHARE_RATIO) + '_single_label_' + str(args.single_label) + '/symbol_' + str(
                args.n_sym_img))
        else:
            output_dir = Path(args.output_dir + '/ckpt_' + args.ta_perform + '/SNR_' + str(args.snr) + '_UE_' + str(
                NUM_UE) + '_share_' + str(SHARE_RATIO) +
